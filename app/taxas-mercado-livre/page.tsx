...
const data = seoLandings.taxas;

export const metadata: Metadata = {
  title: data.title,
  description: data.description,
  alternates: { canonical: site.baseUrl + "/taxas-mercado-livre" },
  openGraph: {
    title: data.title + " | Escala Preço",
    description: data.description,
    url: site.baseUrl + "/taxas-mercado-livre",
  },
  twitter: { title: data.title, description: data.description },
};

export default function TaxasMercadoLivrePage() {
  return (
    <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
      ...
      <div className="mt-10 rounded-xl border border-accent-200 bg-accent-50/50 p-6 text-center">
        <p className="font-medium text-slate-900">Simule as taxas no Escala Preço</p>
        <p className="mt-1 text-sm text-slate-600">
          Use o app para ver o valor líquido estimado na sua categoria.
        </p>
